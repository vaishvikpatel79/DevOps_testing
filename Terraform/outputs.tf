...
output "ecr_image_uri_fastapi_demo_service" {
  description = "Constructed ECR image URI for fastapi-demo-service from service_tags and service_repositories"
  value       = local.service_images["fastapi-demo-service"]
}

output "vpc_id" {
  description = "VPC ID"
  value       = aws_vpc.fastapi_demo_dev_vpc.id
}
...
output "deployment_contract" {
  ...
  value = {
    ...
    network = {
      ...
      subnet_ids          = [aws_subnet.fastapi_demo_public_subnet_1.id, aws_subnet.fastapi_demo_public_subnet_2.id]
      security_group_ids  = [aws_security_group.fastapi_demo_alb_sg.id, aws_security_group.fastapi_demo_ecs_service_sg.id]
    ...
    security = {
      ...
      secret_refs     = null
      role_arns       = { "task_execution_role" = aws_iam_role.fastapi_demo_task_execution_role.arn }
    }
